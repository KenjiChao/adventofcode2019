...
func TestFewestCombinedSteps(t *testing.T) {
	type args struct {
		wire1 []string
		wire2 []string
	}
	...
	for i, tt := range tests {
		t.Run("Test "+strconv.Itoa(i), func(t *testing.T) {
			if got := FewestCombinedSteps(tt.args.wire1, tt.args.wire2); got != tt.want {
				t.Errorf("FewestCombinedSteps() = %v, want %v", got, tt.want)
			}
		})
	}
}